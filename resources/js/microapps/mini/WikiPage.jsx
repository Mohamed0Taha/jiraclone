...
export default function WikiPage({ projectId, viewName }) {
  ...
  return (
    <MicroAppWrapper
      ...
      viewName={viewName}
      appKey="WikiPage"
      defaultValue={{ pages: [], selected: null }}
      title="Wiki Page"
      enableSharing={true}
      defaultShared={true}
    >
      {({ state, setState }) => {
        ...
        const addPage = () => setModalOpen(true);
        ...
        const save = () => {
          setState({ pages: pages.map(p => p.id === (current?.id) ? { ...p, content: draft } : p), selected: current?.id }, { force: true, immediate: true });
        };
          ...
          <Box sx={{ p: 2, display: 'flex', gap: 2 }}>
            ...
            >
              <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1, px: 1 }}>
                ...
                <Button size="small" onClick={addPage} sx={{ borderRadius: 2 }}>Add</Button>
              </Stack>
              <NewDivider/>
              <List dense>
                {pages.map((p) => (
            ...
            <InputModal
              ...
              title="Add Page"
              fields={[{ name: 'title', label: 'Page Title', required: true }]}
