...
export default function OKRTracker({ projectId, viewName }) {
  const [state, setState] = useLocalViewData({ projectId, viewName, appKey: 'OKRTracker', defaultValue: { objectives: [] } });
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Typography variant="h6" fontWeight={600}>OKR Tracker</Typography>
        <Button variant="contained" onClick={addObjective}>Add Objective</Button>
      </Stack>
      <Stack spacing={2}>
        {objectives.length === 0 && (
        ...
        {objectives.map((o) => (
          <Paper key={o.id} variant="outlined" sx={{ p: 2 }}>
            ...
            <Stack spacing={1}>
              ...
              {o.krs.map((k) => (
                <Paper key={k.id} variant="outlined" sx={{ p: 1.25 }}>
