...
export default function Timeline({ project = {}, tasks = {}, users = [] }) {
  ...
  const monthColumns = useMemo(() => {
    ...
    while (current <= dateRange.end) {
      ...
      const monthEnd = endOfMonth(current) > dateRange.end ? dateRange.end : endOfMonth(current);
      months.push({ start: new Date(monthStart), end: new Date(monthEnd), label: formatMonth(current) });
  ...
  const tasksWithEnd = useMemo(() => sortedTasks.filter((t) => t._hasEndDate).length, [sortedTasks]);
  const tasksMissingEnd = useMemo(() => sortedTasks.filter((t) => !t._hasEndDate).length, [sortedTasks]);

  /* ----------------------------- Today marker & actions ----------------------------- */
  const today = new Date();
  const isTodayInRange =
    today >= new Date(dateRange.start.getFullYear(), dateRange.start.getMonth(), dateRange.start.getDate()) &&
    today <= new Date(dateRange.end.getFullYear(), dateRange.end.getMonth(), dateRange.end.getDate());
      ...
      >
        ...
        >
          ...
          <Stack direction="row" alignItems="center" spacing={0.5}>
            ...
            <Typography variant="caption" sx={{ minWidth: 64, textAlign: "center", opacity: 0.7 }}>
              {pixelsPerDay}px/day
            </Typography>
                    ...
                    >
                      ...
                      >
                        <Typography
                          ...
                          sx={{
                            fontWeight: 600,
                            color: task._hasEndDate ? undefined : "#DC2626",
                          }}
                        >
                          {task.title}
                          {!task._hasEndDate && <span style={{ marginLeft: 6 }}>⚠️</span>}
                        </Typography>
                      </Tooltip>
                    </Box>
                  ))}
              ...
              >
                ...
                <Box sx={{ position: "relative", height: "100%" }}>
                  {monthColumns.map((m, i) => {
                    ...
                    const width = (daysBetween(m.start, m.end) + 1) * pixelsPerDay;
                    return (
                      <Box
                        key={`${m.label}-${i}`}
                        sx={{
              ...
              >
                      ...
                      sx={{
                        ...
                        borderLeft: (t) =>
                          `${isStart ? 2 : 1}px dashed ${
                            isStart ? alpha(t.palette.primary.main, 0.5) : alpha(t.palette.divider, 0.5)
                          }`,
                        pointerEvents: "none",
                      }}
                    />
                ...
                <Box sx={{ position: "relative", mt: `${HEADER_HEIGHT}px` }}>
                  {sortedTasks.map((task, idx) => {
                    ...
                    const width = Math.max(MIN_BAR_WIDTH, task._days * pixelsPerDay);
                    ...
                    return (
                      ...
                      >
                        <Box
                          onDoubleClick={() => router.visit(route("tasks.index", project.id))}
                          sx={{
                            position: "absolute",
                            left: startX,
                            top,
                            ...
                            borderRadius: 10,
                            border: (t) => `1px solid ${alpha("#000", 0.08)}`,
        ...
        <Box sx={{ px: { xs: 1.5, md: 3 }, pb: 2, pt: 0.5, opacity: 0.7 }}>
          <Typography variant="caption">
            Tip: Use the Zoom controls to change scale. Double-click a bar to jump back to the board.
          </Typography>
        </Box>
      </Box>
    </>
