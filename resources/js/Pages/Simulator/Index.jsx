...
export default function Index({ auth, simulation, certificationAttempt }) {
        ...
        setTasks(prevTasks => 
            prevTasks.map(task => {
                ...
                if ((task.budget || 0) > 500) {
                    ...
                    console.log(`Normalizing task "${task.title}" budget from ${task.budget} to ${normalizedBudget}`);
    ...
    const attemptPassiveResolutions = useCallback((updatedTasks = tasks) => {
            ...
            for (const ev of unresolved) {
                switch (ev.action_type) {
                    case 'scope_creep':
                        ...
                        if (ev.task_ids?.length) {
                            ...
                            if (t && t.original_estimate) {
                                ...
                                const estimateIncreased = t.estimated_hours > t.original_estimate;
                                const hasHighRemainingHours = t.remaining_hours && t.remaining_hours > (t.original_estimate * 0.8);
                                const lowProgressHighEffort = t.progress < 50 && (t.estimated_hours || t.remaining_hours || 0) > t.original_estimate;
                                const inProgressWithExpansion = t.status === 'In Progress' && t.progress > 0 && (t.remaining_hours > t.original_estimate * 0.5);
                                
                                if (estimateIncreased || hasHighRemainingHours || lowProgressHighEffort || inProgressWithExpansion) {
                                    add.push(ev.id);
                                }
                    ...
                    case 'vendor_delay':
                        ...
                        if (ev.task_ids?.length) {
                            const t = updatedTasks.find(tk=>tk.id===ev.task_ids[0]);
                            if (t && (t.status === 'Pending' || t.delay_handled)) add.push(ev.id);
                        }
                        break;
                    case 'quality_issue':
                        ...
                        if (ev.task_ids?.length) {
                            const t = updatedTasks.find(tk=>tk.id===ev.task_ids[0]);
                            if (t && (t.progress < 5 || t.status === 'In Progress')) add.push(ev.id);
                        }
                        break;
                    case 'budget_request':
                    ...
                    case 'technical_debt':
                        if (ev.task_ids?.length) {
                            const t = updatedTasks.find(tk=>tk.id===ev.task_ids[0]);
                            if (t && t.progress < 90 && t.progress >= 0 && t.status === 'In Progress') add.push(ev.id);
                        }
                        break;
                    case 'add_task':
                    ...
                    case 'team_conflict':
                        ...
                        if (ev.member_ids?.length) {
                            const shared = updatedTasks.filter(t=> t.assignee && ev.member_ids.includes(t.assignee_id||t.assignee));
    ...
    const visibleEvents = useMemo(() => allEvents.filter(e => 
    ...
    useEffect(() => {
        if (allEvents.length > 0) {
            ...
            console.table(allEvents.map(e => ({
                ...
                visible: (e.trigger_week || 0) === currentWeek && !resolvedEventIds.includes(e.id)
            })));
        }
    }, [allEvents, currentWeek, resolvedEventIds]);
    ...
    const [tradeoffStats, setTradeoffStats] = useState({ budget_spent:0,budget_gained:0,morale_up:0,morale_down:0,risk_up:0,risk_down:0,velocity_up:0,velocity_down:0,timeline_up:0,timeline_down:0,actions:0 });

    // Complex action handling for new event system
    const handleComplexAction = useCallback((event, action, evaluation, backendEffects = null) => {
        ...
        const chosen = event.resolution_options?.find(o => (o.key||o.id) === action.resolution_option);
        if (chosen?.follow_up) {
            const m = /(\d+)\s*week/i.exec(chosen.follow_up);
            const offset = m ? parseInt(m[1],10) : 2;
            setPendingFollowUps(prev => [...prev, { token: chosen.follow_up, dueWeek: currentWeek + offset }]);
        }
        ...
        setTradeoffStats(prev => {
            const next = { ...prev };
            if (eff.budget_delta) { eff.budget_delta < 0 ? next.budget_spent += Math.abs(eff.budget_delta) : next.budget_gained += eff.budget_delta; }
            if (eff.team_morale_change) { eff.team_morale_change > 0 ? next.morale_up += eff.team_morale_change : next.morale_down += Math.abs(eff.team_morale_change); }
            if (eff.quality_risk_delta) { eff.quality_risk_delta > 0 ? next.risk_up += eff.quality_risk_delta : next.risk_down += Math.abs(eff.quality_risk_delta); }
            if (eff.velocity_delta_pct) { eff.velocity_delta_pct > 0 ? next.velocity_up += eff.velocity_delta_pct : next.velocity_down += Math.abs(eff.velocity_delta_pct); }
            if (eff.timeline_delta) { eff.timeline_delta > 0 ? next.timeline_up += eff.timeline_delta : next.timeline_down += Math.abs(eff.timeline_delta); }
            next.actions += 1;
            return next;
        });

        // Log the action
        logAction(`Resolved ${event.type}: ${event.title} (Score: ${evaluation.overall_score})`);
    ...
    const evaluateEventResolution = useCallback(async (event, optionKey, notes='') => {
        try {
            const response = await fetch('/simulator/evaluate-action', {
                ...
                body: JSON.stringify({
                    ...
                    game_state: {
                        ...
                        team_utilization: team.length ? team.reduce((s,m)=>s+(m.workload||50),0)/(team.length*100) : 0.5,
                        team_morale_avg: team.length ? team.reduce((s,m)=>s+(m.morale||70),0)/team.length : 70,
            ...
            if (data.success) {
                handleComplexAction(event, { resolution_option: optionKey }, data.evaluation, data.effects);
    ...
    const applyBackendEffects = useCallback((effects) => {
        if (!effects || Object.keys(effects).length === 0) return; // nothing to do
        // Avoid noisy logs in production; only log significant changes
        if (effects.task_updates || effects.team_updates || effects.budget_change || effects.timeline_change) {
            console.debug('Applying backend effects summary', {
                taskUpdates: effects.task_updates ? Object.keys(effects.task_updates).length : 0,
                teamUpdates: effects.team_updates ? Object.keys(effects.team_updates).length : 0,
            ...
            setTasks(prev => prev.map(task => {
                ...
                if (updates) {
                    ...
                    if (updates.progress_boost) {
                        updatedTask.progress = Math.min(100, (task.progress || 0) + updates.progress_boost);
                    }
                    ...
                    } else if (typeof updates.estimated_hours === 'string' && updates.estimated_hours.startsWith('increased_by_') && updates.estimated_hours.endsWith('_percent')) {
                        const m = /increased_by_(\d+)_percent/.exec(updates.estimated_hours);
                        if (m) {
                            const pct = parseInt(m[1],10);
                            updatedTask.estimated_hours = Math.round((updatedTask.estimated_hours || task.estimated_hours || 0) * (1 + pct/100));
                        }
                    }
                    if (updates.estimated_hours === 'increased_by_20_percent') {
                        updatedTask.estimated_hours = Math.floor((task.estimated_hours || 0) * 1.2);
                    }
                    if (updates.estimated_hours === 'increased_by_30_percent') {
                        updatedTask.estimated_hours = Math.floor((task.estimated_hours || 0) * 1.3);
                    }
                    if (updates.estimated_hours === 'increased_by_50_percent') {
                        updatedTask.estimated_hours = Math.floor((task.estimated_hours || 0) * 1.5);
                    }
                    ...
                    if (updates.assignee === 'skill_expert') {
                        ...
                        const expert = team.find(m => 
                            taskSkills.some(skill => (m.skills || []).includes(skill)) &&
                            (m.morale || 70) > 50
                        );
                        ...
                        const neutralMember = team.find(m => 
                            !conflictMemberIds.includes(m.id) && (m.morale || 70) > 60
                        );
                        updatedTask.assignee = neutralMember ? neutralMember.name : task.assignee;
                    ...
                    Object.keys(updates).forEach(key => {
                        if (!['progress_boost', 'progress', 'estimated_hours', 'priority', 'assignee'].includes(key)) {
            ...
            setTeam(prev => prev.map(member => {
                ...
                if (updates) {
                    ...
                    if (updates.morale_change) {
                        updatedMember.morale = Math.min(100, Math.max(0, (member.morale || 70) + updates.morale_change));
                    }
        ...
        if (effects.member_reassign_from) {
            setTasks(prev => prev.map(t => {
                ...
                if (member && t.assignee === member.name) {
                    return { ...t, assignee: null, status: t.status === 'In Progress' ? 'Pending' : t.status, reassignment_needed: true };
                }
                return t;
            }));
        ...
        if (effects.member_departure_id) {
            ...
            setTasks(prev => prev.map(t => (departed && t.assignee === departed.name) ? { ...t, assignee: null, status: t.status === 'In Progress' ? 'Pending' : t.status, capacity_loss: true } : t));
            logAction('Member departed (capacity impact applied)');
        }
    }, [adjustmentsRef, spawnCoinBurst, logAction, currentWeek, team, setRemainingBudget, totalBudget, calcSpent, tasks]);

    // Apply specific resolutions based on the event type and action taken
    const applySpecificEventResolution = useCallback((event, action, evaluation) => {
                ...
                case 'cancel_low_priority':
                    ...
                    setTasks(prev => prev.map(task => 
                        task.priority === 'Low' || affectedTaskIds.includes(task.id)
                            ? { ...task, status: 'Cancelled', cancellation_reason: 'Budget constraints' }
                            : task
                    ));
                    // Refund unused budget from cancelled tasks
                    const cancelledBudget = tasks.filter(t => 
                        (t.priority === 'Low' || affectedTaskIds.includes(t.id)) && t.status !== 'Cancelled'
                    ).reduce((sum, t) => sum + ((t.budget || 0) * (100 - (t.progress || 0)) / 100), 0);
                ...
                case 'reduce_scope_across_tasks':
                    ...
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id) || task.status === 'Pending'
                            ? { 
                                ...task, 
                                estimated_hours: Math.floor((task.estimated_hours || 0) * 0.75),
                                budget: Math.floor((task.budget || 0) * 0.8),
                                scope_reduced: true,
                                scope_reduction_notes: '25% scope reduction due to budget constraints'
                            }
                            : task
                    ));
                    break;
                    ...
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id)
                            ? { 
                                ...
                                status: 'Pending',
                                deferred: true,
                                deferral_reason: 'Moved to Phase 2 due to budget constraints',
        ...
        else if (eventType === 'Member Sickness' || eventType === 'Member Unavailable' || event.title?.includes('unavailable')) {
                ...
                case 'redistribute_workload':
                    ...
                    setTasks(prev => prev.map(task => {
                        if (task.assignee === memberName && task.status !== 'Completed') {
                            ...
                            const availableMembers = team.filter(m => 
                                m.id !== affectedMemberIds[0] && 
                                (m.morale || 70) > 50 &&
                                (task.required_skills || []).some(skill => (m.skills || []).includes(skill)) ||
                                (task.required_skills || []).includes('General')
                            );
                            
                            if (availableMembers.length > 0) {
                                const newAssignee = availableMembers[Math.floor(Math.random() * availableMembers.length)];
                                return {
                                    ...task,
                        ...
                        setTasks(prev => prev.map(task => 
                            task.assignee === memberName && affectedTaskIds.includes(task.id)
                                ? { 
                                    ...
                                    estimated_hours: Math.floor((task.estimated_hours || 0) * 0.9) // Specialists work faster
                                }
                                : task
                        ));
                    } else {
                        ...
                        setTasks(prev => prev.map(task => 
                            task.assignee === memberName && affectedTaskIds.includes(task.id)
                                ? { 
                                    ...
                                    status: 'Pending',
                                    assignee: null,
                                    delay_reason: 'Insufficient budget for temporary help'
                                }
                                : task
                        ));
                    }
                ...
                setTeam(prev => prev.map(member => 
                    affectedMemberIds.includes(member.id)
                        ? { 
                            ...
                            availability_reason: actionKey === 'approve_sick_leave' ? 'Sick Leave' : 'Time Off',
        ...
        // Budget Cut Events (NEW MANDATORY EVENT TYPE)
            ...
            const costReduction = 0.20; // 20% budget reduction
            ...
            switch (actionKey) {
                ...
                case 'defer_tasks':
                    ...
                    setTasks(prev => prev.map((task, index) => 
                        index < affectedTaskCount && task.status !== 'Done' && task.priority !== 'High'
                            ? { 
                                ...task, 
                                status: 'Pending',
                                deferred: true,
        ...
        else if (eventType === 'Member Demands' || event.title?.includes('Compensation Request')) {
            const affectedMember = team.find(m => affectedMemberIds.includes(m.id));
        ...
        // Skill Gap Events
            ...
            switch (actionKey) {
                case 'consolidate_to_expert':
                    ...
                    const skillRequired = event.desc?.match(/(\w+) expertise/)?.[1] || 'General';
                    const expert = team.find(m => 
                    ...
                    if (expert) {
                        ...
                        setTeam(prev => prev.map(member => 
                            member.id === expert.id
                                ? { ...member, morale: Math.max(0, (member.morale || 70) - 10), workload: 'High' }
                                : member
                        ));
                    }
                    break;
                ...
                case 'implement_mentoring':
                    ...
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id)
                            ? { 
                                ...
                                estimated_hours: Math.floor((task.estimated_hours || 0) * 1.2), // 20% overhead initially
                                skill_transfer: true
                            }
                            : task
                    ));
                    ...
                    setTeam(prev => prev.map(member => 
                        !affectedMemberIds.includes(member.id) // Non-experts get skill boost
                            ? { 
                                ...member, 
                                skills: [...(member.skills || []), skillRequired].filter((s, i, arr) => arr.indexOf(s) === i),
        ...
        // Team Conflict Events
            ...
            switch (actionKey) {
                ...
                case 'reassign_task':
                    ...
                    const neutralMembers = team.filter(m => !affectedMemberIds.includes(m.id));
                    if (neutralMembers.length > 0) {
                        setTasks(prev => prev.map(task => 
                            affectedTaskIds.includes(task.id)
                                ? { 
                                    ...task, 
                                    assignee: neutralMembers[Math.floor(Math.random() * neutralMembers.length)].name,
                    ...
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id)
                            ? { 
                                ...
                                estimated_hours: Math.floor((task.estimated_hours || 0) * 1.15) // 15% overhead for reviews
                            }
                            : task
                    ));
                    break;
                ...
                case 'implement_changes':
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id)
                            ? { 
                                ...task, 
                                status: task.status === 'Completed' ? 'In Progress' : task.status, // Reopen if needed
                                progress: Math.max(0, (task.progress || 0) - 20), // Rework required
                                client_feedback_incorporated: true,
                                estimated_hours: Math.floor((task.estimated_hours || 0) * 1.3) // 30% more work
                            }
                            : task
                    ));
                    break;
                    ...
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id)
                            ? { 
                                ...
                                technical_debt: true,
                                estimated_hours: Math.floor((task.estimated_hours || 0) * 1.1) // 10% overhead
                            }
                            : task
                    ));
                    break;
                        ...
                        setTasks(prev => prev.map(task => 
                            affectedTaskIds.includes(task.id)
                                ? { 
                                    ...
                                    estimated_hours: Math.floor((task.estimated_hours || 0) * 0.85) // 15% efficiency gain
                                }
                                : task
                        ));
                    } else {
                    ...
                    setTasks(prev => prev.map(task => 
                        affectedTaskIds.includes(task.id)
                            ? { 
                                ...task, 
                                status: 'Pending',
                                delay_reason: 'Infrastructure issues - waiting for resolution',
                                delayed_weeks: 1
                            }
                            : task
                    ));
        ...
        if (evaluation.metrics_impact) {
            ...
            if (evaluation.metrics_impact.team_morale_change) {
                setTeam(prev => prev.map(member => ({
                    ...member,
                    morale: Math.min(100, Math.max(0, (member.morale || 70) + evaluation.metrics_impact.team_morale_change))
                })));
            }
        }
    ...
    const applyActionConsequences = useCallback((consequences, metricsImpact) => {
        ...
        if (metricsImpact?.team_morale_change) {
            setTeam(prev => prev.map(member => ({
                ...member,
                morale: Math.min(100, Math.max(0, (member.morale || 70) + metricsImpact.team_morale_change))
            })));
        }
        ...
        consequences.forEach(consequence => {
            switch (consequence.type) {
                case 'timeline_delay':
                    ...
                    setTasks(prev => prev.map(task => ({
                        ...task,
                        estimated_hours: task.estimated_hours ? task.estimated_hours * 1.1 : task.estimated_hours
                    })));
                    break;
                case 'velocity_boost_future':
                    // Could improve future task progress rates
                    setTasks(prev => prev.map(task => task.status === 'In Progress' ? {
                        ...task,
                        progress: Math.min(100, (task.progress || 0) + 5)
                    } : task));
                    break;
                case 'quality_assurance':
                    ...
                    setTasks(prev => prev.map(task => task.priority === 'High' ? {
                        ...task,
                        quality_assured: true
                    } : task));
                    break;
    ...
    const spent = calcSpent(tasks);
        const remainingBudget = Math.max(0, totalBudget - spent - overheadCostsRef.current + adjustmentsRef.current);
        
        const avgMorale = team.length > 0 ? team.reduce((sum, m) => sum + (m.morale || 70), 0) / team.length : 70;
        const teamUtilization = team.length > 0 ? team.reduce((sum, m) => sum + (m.workload || 50), 0) / (team.length * 100) : 0.5;
    ...
    const attemptAdvanceWeek = () => {
        ...
        setCurrentWeek(w => {
            ...
            const oldStandups = allEvents.filter(e => 
    ...
    const relatedEvents = allEvents.filter(e => ['update_task','reassign_task'].includes(e.action_type) && (e.task_ids||[]).includes(updated.id) && !e.resolution_requires_action);
        ...
        allEvents.forEach(ev => {
            if (ev.resolution_rule?.must_cancel_one_of_task_ids && ev.action_type === 'update_task' && !resolvedEventIds.includes(ev.id) && !ev.resolution_requires_action) {
    ...
    const handleTaskAction = useCallback(async (taskId, actionType, params) => {
        try {
            ...
            switch (actionType) {
                case 'split_task':
                    if (task.estimated_hours <= 8 || task.progress >= 50) {
                        return { success: false, message: 'Task cannot be split (too small or too advanced)' };
                    }
                ...
                case 'add_resources':
                    ...
                    if (remainingBudget + budgetImpact < 0) {
                        return { success: false, message: 'Insufficient budget for additional resources' };
                    }
                    
                    result.taskUpdates = {
                        additional_resources: additionalMembers,
                        resource_type: resourceType,
                        estimated_hours: Math.max(4, task.estimated_hours - (additionalMembers * 8))
                    };
                ...
                case 'change_scope':
                    if (task.progress >= 30) {
                        return { success: false, message: 'Cannot change scope - task too advanced' };
                    }
                    ...
                    switch (scopeChange) {
                        case 'reduce':
                            effortModifier = 0.75;
                            budgetModifier = 0.75;
                            result.message = 'Scope reduced by 25% - faster delivery, less comprehensive';
                            break;
                        case 'expand':
                            effortModifier = 1.4;
                            budgetModifier = 1.4;
                            budgetImpact = -((task.budget || 0) * 0.4);
                            result.message = 'Scope expanded by 40% - more comprehensive, higher cost';
                            break;
                        case 'refocus':
                            result.message = 'Task priorities refocused - optimized for current needs';
                            break;
                    }
                    
                    result.taskUpdates = {
                        estimated_hours: Math.floor((task.estimated_hours || 0) * effortModifier),
                ...
                case 'rush_delivery':
                    if (task.status !== 'In Progress') {
                        return { success: false, message: 'Task must be in progress to rush delivery' };
                    }
                    ...
                    switch (rushStrategy) {
                        case 'overtime':
                            ...
                            teamEffects = [{ type: 'morale_decrease', amount: 5, reason: 'overtime_pressure' }];
                            result.message = 'Overtime scheduled - task accelerated but team morale decreased';
                            break;
                        case 'parallel':
                            ...
                            result.message = 'Parallel development initiated - faster but higher risk';
                            break;
                        case 'mvp':
                            result.taskUpdates = {
                            ...
                            result.message = 'MVP delivery prioritized - core functionality delivered first';
                            break;
                    }
                    break;
                ...
                case 'handle_delays':
                    ...
                    const delayEvents = allEvents.filter(e => 
                    ...
                    switch (delayStrategy) {
                        case 'expedite':
                            ...
                            result.message = 'Expedited fee paid - vendor delays resolved, progress boosted';
                            break;
                        case 'parallel':
                            ...
                            result.message = 'Parallel contingency started - delays bypassed but risk increased';
                            break;
                        case 'absorb':
                            ...
                            teamEffects = [{ type: 'morale_decrease', amount: 3, reason: 'timeline_pressure' }];
                            result.message = 'Delay absorbed - timeline extended, slight team morale decrease';
                            break;
                        case 'reschedule':
                            ...
                            result.message = 'Dependencies rescheduled - delay managed with coordination cost';
                            break;
                    }
                    break;
                ...
                case 'request_budget':
                    ...
                    const budgetEvents = allEvents.filter(e => 
                    ...
                    console.log('Budget request debug:', {
                        ...
                        budgetEventsFound: budgetEvents.map(e => ({ id: e.id, action_type: e.action_type, task_ids: e.task_ids }))
                    });
                    
                    if (budgetEvents.length > 0) {
                        // Try to resolve via the backend evaluation system
                        const targetEvent = budgetEvents[0];
                        const approvalOption = targetEvent.resolution_options?.find(opt => 
                            opt.key === 'approve' || opt.title?.toLowerCase().includes('approve')
                        );
                        
                        if (approvalOption) {
                            try {
                                const actionResponse = await fetch('/simulator/evaluate-action', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                    ...
                                    body: JSON.stringify({
                                        ...
                                        game_state: {
                                            remaining_budget: remainingBudget,
                                            team_morale_avg: team.reduce((sum, m) => sum + (m.morale || 70), 0) / team.length,
                                            timeline_buffer_weeks: Math.max(0, totalWeeks - currentWeek),
                                ...
                                console.log('Backend budget request failed, resolving locally:', error);
                                // Fallback: resolve locally if backend fails
                                resolveEvent(targetEvent.id);
                                result.message = 'Budget request submitted - awaiting management review';
                            }
                        } else {
                            // No approval option found, resolve anyway since user took action
                            console.log('No approval option found, resolving budget event anyway');
                            resolveEvent(targetEvent.id);
                            result.message = 'Budget request submitted - will be reviewed by management';
                        }
                    } else {
                ...
                setTeam(prev => prev.map(member => {
                    ...
                    teamEffects.forEach(effect => {
                        if (effect.type === 'morale_decrease') {
                            updatedMember.morale = Math.max(0, (updatedMember.morale || 70) - effect.amount);
                        } else if (effect.type === 'morale_increase') {
                            updatedMember.morale = Math.min(100, (updatedMember.morale || 70) + effect.amount);
                        }
                    });
                    return updatedMember;
                }));
    ...
    }, [tasks, currentWeek, remainingBudget, totalBudget, team, calcSpent, allEvents, resolvedEventIds, resolveEvent, applyBackendEffects, totalWeeks]);
    ...
    const toResolve = allEvents.filter(e => ['team_event','morale_slump'].includes(e.action_type) && (e.trigger_week||0) === currentWeek && !resolvedEventIds.includes(e.id)).map(e=>e.id);
    if (toResolve.length) setResolvedEventIds(prev => [...prev, ...toResolve.filter(id=>!prev.includes(id))]);
    };
    ...
    const handleStandup = () => {
        ...
        console.log('Tasks budgets:', tasks.map(t => ({ title: t.title, budget: t.budget, progress: t.progress })));
    ...
    useEffect(()=>{
        setTasks(prev=> prev.map(t=> t.original_estimate ? t : { ...t, original_estimate: t.estimated_hours }));
    },[]);
    ...
    const computeResults = useCallback(() => {
        ...
        const actionParticipation = allEvents.filter(e => e.resolution_options && e.resolution_options.length > 0).length;
        const actionCompletionRate = actionParticipation > 0 ? (actionCount / actionParticipation) : 0;
        ...
        const scoreBudget = (() => {
            ...
            if (remainingPct < sweetMin) return 50 * Math.max(0, 1 - (sweetMin - remainingPct)*2.5);
            if (remainingPct > sweetMax) return 50 * Math.max(0, 1 - (remainingPct - sweetMax)*2.0);
        ...
        const scoreStability = 10 * Math.max(0, 1 - removals / Math.max(1, simulation?.team?.length || 1)); // reduced from 15
        ...
        let raw = scoreBudget + scoreDelivery + scoreHighFocus + scoreAdapt + scoreMorale + scoreStability + scoreActionQuality + scoreActionEngagement;
    ...
    }, [tasks, totalBudget, remainingBudget, team, projectWeeks, simulation, activityLog, actionCount, totalScore, allEvents]);

    // Submit certification score when simulation completes
    const submitCertificationScore = useCallback(async (simulationScore) => {
            ...
            const response = await fetch('/certification/complete-from-simulator', {
                ...
                body: JSON.stringify({
                    ...
                    simulation_data: {
                        ...
                        team_final_morale: team.reduce((sum, m) => sum + (m.morale || 70), 0) / team.length,
    ...
    }, [certificationAttempt, project, projectWeeks, remainingBudget, totalBudget, tasks, team, actionCount, resolvedEventIds, activityLog]);
        ...
        <Layout {...layoutProps}>
                ...
                <div ref={navRef} style={{ display: 'flex', alignItems: 'center', gap: '8px', position: 'relative' }}>
                    ...
                    {actionCount > 0 && (
                        <div style={{ 
                            ...
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems: 'center', marginTop: 2 }}>
                                ...
                                <span style={{ 
                                    color: actionCount > 0 ? (
                                        (totalScore / actionCount) >= 80 ? '#2e7d32' : 
                                        (totalScore / actionCount) >= 60 ? '#ed6c02' : '#d32f2f'
                                    ) : dashboardAccent 
                                }}>
                    ...
                    {tradeoffStats.actions > 0 && (
                        <div style={{ 
                            ...
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems: 'center', marginTop: 2 }}>
                                <span>+${(tradeoffStats.budget_gained/1000).toFixed(1)}k</span>
                                <span style={{color:'#d32f2f'}}>-{(tradeoffStats.budget_spent/1000).toFixed(1)}k</span>
                            </div>
                        </div>
                    )}
            ...
            <div style={{ 
                ...
                fontSize: `${1.15 * fontScale}rem`
                                ...
                                <div ref={eventsRef} style={{ flex:'0 0 30%', display:'flex', flexDirection:'column', background: dashboardCardGradient, borderRight:'2px solid #eaeaea', position:'relative', minHeight:0 }}>
                                    ...
                                    {walkStep === 1 && (
                                        <div style={{ position:'absolute', left:8, right:8, bottom:8 }}>
                                            <GuideOverlay onNext={advanceWalk} onSkip={skipWalk} title="Events" description="Assess and resolve events directly here. Select an event and choose an option inline." />
                                        </div>
                                    )}
                                </div>
                    ...
                    <div style={{ flex:1, overflowY:'auto', minHeight:0, paddingRight:4 }}>
                        <Tasks 
                            ...
                            simulationState={{
                                ...
                                teamMorale: team.length > 0 ? team.reduce((sum, m) => sum + (m.morale || 70), 0) / team.length : 70,
                            ...
                            events={allEvents}
                            resolvedEventIds={resolvedEventIds}
                            onResolveEventAction={async (event, resolutionOption, notes='') => {
                                try {
                                    const response = await fetch('/simulator/evaluate-action', {
                                        method: 'POST',
                                        headers: {
                                    ...
                                    const data = await response.json();
                                    if (data.success) {
                                        handleComplexAction(event, { resolution_option: resolutionOption }, data.evaluation, data.effects);
                                        resolveEvent(event.id);
                                        return { success:true, evaluation:data.evaluation, effects:data.effects };
                                    }
                                    return { success:false, message:data.message || 'Evaluation failed' };
            ...
            <Dialog open onClose={()=>{
                ...
                <DialogContent dividers>
                    ...
                    {certificationAttempt && (
                        <Box sx={{ mt: 2, p: 2, bgcolor: results.overall >= 70 ? 'success.light' : 'warning.light', borderRadius: 1 }}>
                            ...
                            {(() => {
                                try {
                                    const certResult = JSON.parse(sessionStorage.getItem('certificationResult') || '{}');
                                    if (certResult.success !== undefined) {
                                        return (
                                            <Typography variant="body2" sx={{ mt: 1 }}>
                                                <strong>Final Result: </strong>
                                                {certResult.passed ? '🎉 Certification Granted!' : '❌ Certification Not Granted'}
                                                <br />
                                                Theory: {certResult.theory_percentage}% | Simulator: {certResult.simulation_percentage}%
                                            </Typography>
                                        );
                                    }
                                } catch (e) {}
                            ...
                            {(() => {
                                try {
                                    ...
                                    if (certResult.passed) {
                                        return (
                                            <>
                                                <MUIButton onClick={() => window.location.href = '/certification/certificate'} variant="contained">
                                                    View Certificate
                                                </MUIButton>
                                                <MUIButton onClick={() => window.location.href = '/certification/badge'}>
                                                    View Badge
                                                </MUIButton>
                                            </>
                                        );
                                ...
                                return (
                                    <MUIButton onClick={() => window.location.href = '/certification'} variant="contained">
