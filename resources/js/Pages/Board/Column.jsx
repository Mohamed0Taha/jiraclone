...
export default function Column({
  ...
  const shouldShowSummary =
    typeof showProjectSummary === "boolean"
      ? showProjectSummary
      : statusKey === "todo";
  ...
  const ProjectSummary = ({ project }) => {
    ...
    const chips = [
      { icon: <KeyRoundedIcon fontSize="small" />, label: project.key || "No key", show: true },
      { icon: <CategoryRoundedIcon fontSize="small" />, label: [m.project_type, m.domain].filter(Boolean).join(" · "), show: m.project_type || m.domain },
      { icon: <PlaceRoundedIcon fontSize="small" />, label: [m.area, m.location].filter(Boolean).join(" · "), show: m.area || m.location },
      { icon: <GroupsRoundedIcon fontSize="small" />, label: m.team_size ? `${m.team_size} members` : null, show: !!m.team_size },
      { icon: <LocalAtmRoundedIcon fontSize="small" />, label: m.budget ? `${m.budget}` : null, show: !!m.budget },
      { icon: <BusinessRoundedIcon fontSize="small" />, label: m.primary_stakeholder || null, show: !!m.primary_stakeholder },
      {
        icon: <CalendarMonthRoundedIcon fontSize="small" />,
        label: project.start_date || project.end_date ? `${project.start_date || "—"} → ${project.end_date || "—"}` : null,
    ...
    return (
      ...
      >
        <Stack spacing={0.75}>
          <Typography variant="subtitle2" sx={{ fontWeight: 800, letterSpacing: 0.1, color: alpha(theme.palette.text.primary, 0.92), lineHeight: 1.25 }}>
            {project.name}
          </Typography>
            ...
            <Stack spacing={0.4}>
              {m.objectives && (
                <Stack direction="row" spacing={0.5} alignItems="flex-start">
                  <FlagRoundedIcon fontSize="small" sx={{ mt: "2px", color: alpha(theme.palette.success.main, 0.9) }} />
              ...
              {m.constraints && (
                <Stack direction="row" spacing={0.5} alignItems="flex-start">
                  <InfoRoundedIcon fontSize="small" sx={{ mt: "2px", color: alpha(theme.palette.warning.main, 0.9) }} />
          ...
          >
            ...
            <Stack
              ...
              sx={{
                ...
                "&::-webkit-scrollbar-thumb": { borderRadius: 6, backgroundColor: alpha(metaConf.accent, 0.45) },
              }}
            >
            ...
            <Box sx={{ pt: 0.6, mt: 0.35, borderTop: `1px dashed ${alpha(metaConf.accent, 0.4)}` }}>
              <Button
                size="small"
                variant="text"
