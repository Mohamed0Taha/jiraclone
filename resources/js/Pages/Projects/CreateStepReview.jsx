    ...
    <Stack spacing={3}>
      ...
      <Box>
        ...
        <Stack spacing={1.5}>
          ...
          <ReviewLine label="Team Size" value={`${data.meta.team_size} ${data.meta.team_size === 1 ? 'person' : 'people'}`} />
      ...
      <Box sx={{ pt: 2 }}>
        <Stack direction="row" spacing={1} flexWrap="wrap">
          ...
          {data.start_date && (
            <Chip 
              label={`Starts ${formatDate(data.start_date)}`} 
              variant="outlined" 
            />
          )}
          {data.meta.team_size > 1 && (
            <Chip 
              label={`${data.meta.team_size} team members`} 
              variant="outlined" 
            />
          )}
        </Stack>
