...
class AssistantBulkFiltersTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $tg = Mockery::mock(TaskGeneratorService::class);
        $tg->shouldIgnoreMissing();
        $tg->shouldReceive('generateTasks')->andReturn([]);
        $this->cps = new CommandProcessingService($openAIMock, new ProjectContextService, $tg);
    ...
    public function test_bulk_filter_status_priority_matrix()
    {
        ...
        foreach ($statuses as $s) {
            foreach ($priorities as $p) {
                $q = "List $p priority $s tasks";
                $ans = $this->qas->answer($this->project, $q, [], null);
                $this->assertIsString($ans);
                $checked++;
                if ($checked >= 20) {
                    break 2;
                }
