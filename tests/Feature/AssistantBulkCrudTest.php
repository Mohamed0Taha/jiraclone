...
class AssistantBulkCrudTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $openAIMock = Mockery::mock(OpenAIService::class); $openAIMock->shouldIgnoreMissing(); $openAIMock->shouldReceive('chatText')->andReturn('Mocked');
        $this->qas = new QuestionAnsweringService($openAIMock,new ProjectContextService());
        $taskGenMock = Mockery::mock(TaskGeneratorService::class); $taskGenMock->shouldIgnoreMissing(); $taskGenMock->shouldReceive('generateTasks')->andReturn([]);
    ...
    public function test_mass_crud_and_references_suite()
    {
        ...
        $basePhrases = ['Move','Set','Mark']; $targets = ['to done','to review','to in progress'];
        $i=0; $total=500; $taskId = Task::where('project_id',$this->project->id)->first()->id;
        while($i<$total){
