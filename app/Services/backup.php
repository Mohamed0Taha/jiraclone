...
SYS;
    ...
    private function sanitizeAnswer(string $text): string
    {
        ...
        if (mb_strlen($t) > 800) $t = mb_substr($t, 0, 800) . '…';
        return trim($t);
    }
    ...
    private function openAiChatText(array $messages, float $temperature = 0.2): string
    {
        ...
        if (!$res->ok()) throw new Exception('OpenAI chat request failed');
    ...
    private function classifyIntentEnhanced(string $message): string
    {
        ...
        foreach (self::QUESTION_PATTERNS as $pattern) {
            if (preg_match($pattern, $m)) {
                if (!$this->hasStrongActionVerb($m)) return 'question';
            }
        }

        foreach (self::ACTION_PATTERNS as $patterns) {
            foreach ($patterns as $pattern) {
                if (preg_match($pattern, $m)) return 'action';
            }
        }

        if ($this->isLikelyQuestion($m)) return 'question';
        return 'action';
    }
    ...
    private function hasStrongActionVerb(string $message): bool
    {
        ...
        foreach ($verbs as $verb) if (preg_match('/\b'.$verb.'\b/i', $message)) return true;
        return false;
    }
    ...
    private function isLikelyQuestion(string $message): bool
    {
        ...
        foreach ($indicators as $i) if (strpos($message, $i) !== false) return true;
        return false;
    }
    ...
    private function compileActionPlanEnhanced(Project $project, string $message, array $history = []): array
    {
        ...
        if ($ordinal) {
            ...
            if (!empty($ctxPlan)) {
                if (in_array($ctxPlan['type'], ['bulk_update','bulk_assign','bulk_delete'], true)) {
                    ...
                    if (empty($guessed)) $guessed = $ctxPlan['updates'] ?? $ctxPlan['changes'] ?? [];
                    if (!empty($guessed)) {
        ...
        if (preg_match('/\b(?:create|add|new|make)\s+(?:a\s+)?(?:new\s+)?task\s+(?:called\s+|named\s+|titled\s+)?["\']?(.+?)["\']?$/i', $m, $matches)) {
            ...
            if ($title !== '') return $this->createTaskPlan($title);
        }

        // "Task: Title"
        if (preg_match('/^task:\s*(.+)$/i', $m, $matches)) {
            $potentialTitle = trim($matches[1]);
            if (strlen($potentialTitle) > 2) return $this->createTaskPlan($potentialTitle);
        }
        ...
        if (preg_match('/#?(\d+)/', $m, $idMatch)) {
            ...
            if (!empty($updates)) return $this->createTaskUpdatePlan($taskId, $updates);
        }
        ...
        if (!empty($updates)) {
            if (empty($filters)) $filters = ['all' => true];
            if ($ordinal) $filters = array_merge($filters, $ordinal);
        ...
        if (preg_match('/\bassign\b.*?\bto\s+([a-z0-9._@\- ]+)/i', $m, $matches)) {
            ...
            if ($ordinal) $plan['filters'] = array_merge($plan['filters'] ?? [], $ordinal);
            return $plan;
        }
    ...
    private function parseTaskUpdates(Project $project, string $message, int $taskId): array
    {
        ...
        if (preg_match('/\b(?:due|deadline)\s+(?:on|by|at)?\s*(.+?)(?:\s|$)/i', $m, $matches)) {
            ...
            if ($date) $updates['end_date'] = $date->toDateString();
        }
    ...
    private function parseFiltersEnhanced(Project $project, string $message): array
    {
        ...
        if (strpos($m, 'overdue') !== false) $filters['overdue'] = true;
        if (preg_match('/\b(?:unassigned|not\s+assigned|no\s+assignee)\b/i', $m)) $filters['unassigned'] = true;
        ...
        if (preg_match('/\ball\s+tasks?\b/i', $m)) $filters['all'] = true;
    ...
    private function synthesizeWithLLMEnhanced(Project $project, string $message, array $history = []): array
    {
        ...
        try {
            ...
            if (!is_array($plan) || empty($plan['type'])) return ['type' => null];

            $plan = $this->normalizeLLMPlan($project, $plan);
            if (!empty($plan['type'])) $plan['type'] = $this->normalizeType($plan['type']);
            $validate = $this->validatePlan($project, $plan);
            if ($validate['_ok'] === false) return ['type' => null];
            return $plan;
        } catch (Throwable $e) {
    ...
    private function parseDeleteCommand(Project $project, string $message): array
    {
        ...
        if (preg_match('/#?(\d+)/', $message, $matches)) {
            ...
            if (!$task) return ['_error' => "Task #{$taskId} not found in this project."];
            return ['type' => 'task_delete', 'selector' => ['id' => $taskId]];
        }

        if (preg_match('/\b(?:all|everything)\b/i', $m)) return ['type' => 'bulk_delete_all'];
        if (strpos($m, 'overdue') !== false) return ['type' => 'bulk_delete_overdue'];

        $filters = $this->parseFiltersEnhanced($project, $message);
        if (!empty($filters)) return ['type' => 'bulk_delete', 'filters' => $filters];
    ...
    private function getProjectOverview(Project $project): array
    {
        ...
        if ($snap['tasks']['overdue'] > 0) $message .= "⚠️ Overdue Tasks: {$snap['tasks']['overdue']}\n";

        $priorities = $this->getTasksByPriority($project);
        $message .= "\nBy Priority:\n";
        foreach ($priorities as $priority => $count) {
            if ($count > 0) $message .= "• ".ucfirst($priority).": {$count}\n";
        }
    ...
    private function wantsOverview(string $message): bool
    {
        ...
        foreach ($patterns as $p) if (preg_match($p, $m)) return true;
        return false;
    }
    ...
    private function execBulkDeleteAll(Project $project): array
    {
        ...
        $count = 0;
        foreach ($tasks as $t) { $t->delete(); $count++; }
    ...
    private function applyUpdatesToTask(Project $project, Task $task, array $updates): bool
    {
        ...
        if (isset($updates['priority'])) {
            ...
            if ($prio && $task->priority !== $prio) { $task->priority = $prio; $changed = true; }
        }
    ...
    private function buildTaskQuery(Project $project, array $filters)
    {
        ...
        if (!empty($filters['title_hints']) && is_array($filters['title_hints'])) {
            ...
            if (empty($idsFromTitles)) { $q->whereRaw('1=0'); return $q; }
        ...
        if (!empty($filters['title_contains']) && is_array($filters['title_contains'])) {
            $q->where(function($sub) use ($filters) {
                foreach ($filters['title_contains'] as $needle) $sub->orWhere('title','LIKE','%'.$needle.'%');
            });
        }

        if (!empty($filters['description_contains']) && is_array($filters['description_contains'])) {
            $q->where(function($sub) use ($filters) {
                foreach ($filters['description_contains'] as $needle) $sub->orWhere('description','LIKE','%'.$needle.'%');
            });
        }
        ...
        if (!empty($filters['due_before'])) $q->whereDate('end_date','<=',$filters['due_before']);
        if (!empty($filters['due_after']))  $q->whereDate('end_date','>=',$filters['due_after']);
        if (!empty($filters['due_on']))     $q->whereDate('end_date','=',$filters['due_on']);

        if (!empty($filters['created_before'])) $q->whereDate('created_at','<=',$filters['created_before']);
        if (!empty($filters['created_after']))  $q->whereDate('created_at','>=',$filters['created_after']);

        $orderBy = $filters['order_by'] ?? 'created_at';
        $order   = strtolower($filters['order'] ?? 'asc');
        if (!in_array($orderBy, ['created_at','updated_at','end_date','priority','id','status','title'], true)) $orderBy = 'created_at';
    ...
    private function countAffected(Project $project, array $filters): int
    {
        ...
        if (!empty($filters['limit'])) return min($base, max(1, (int)$filters['limit']));
        return $base;
    }
    ...
    private function countOverdue(Project $project): int
    {
        ...
        foreach ($q->get() as $t) {
            try { if (Carbon::parse($t->end_date)->isPast()) $count++; } catch (\Throwable $e) {}
        }
        return $count;
    }
    ...
    private function buildSnapshot(Project $project): array
    {
        ...
        foreach ($project->tasks as $t) {
            ...
            if (!empty($t->end_date)) {
                try {
                    if (Carbon::parse($t->end_date)->isPast() && $s !== 'done') $overdue++;
                } catch (\Throwable $e) {}
            }
        }
    ...
    private function resolveTaskIdsByTitles(Project $project, array $titleHints): array
    {
        ...
        foreach ($titleHints as $hint) {
            ...
            if ($exact) { $ids[] = (int)$exact->id; continue; }

            $contains = $tasks->first(fn($t) => mb_strpos(mb_strtolower((string)$t->title), $hintLower) !== false);
            if ($contains) { $ids[] = (int)$contains->id; continue; }

            $bestId = null; $bestScore = 0.0;
            foreach ($tasks as $t) {
                $score = $this->similarityScore((string)$t->title, $hintLower);
                if ($score > $bestScore) { $bestScore = $score; $bestId = (int)$t->id; }
            }
            if ($bestId !== null && $bestScore >= 0.55) $ids[] = $bestId;
        }
    ...
    private function similarityScore(string $a, string $b): float
    {
        ...
        if (mb_strpos($a, $b) !== false) $score = max($score, min(1.0, (mb_strlen($b) / (float)$len)));
        return $score;
    }
    ...
    private function resolveStatusToken(Project $project, string $token): ?string
    {
        ...
        if (in_array($t, ['first','first stage','first column','left','leftmost','start','begin','beginning'])) return 'todo';
        if (in_array($t, ['last','last stage','last column','final','end','right','rightmost'])) return 'done';
    ...
    private function resolvePriorityToken(string $token): ?string
    {
        ...
        if (preg_match('/^(low|medium|high|urgent)\s+priority$/', $t, $m)) return $m[1];
        return null;
    }
    ...
    private function projectStatusAliasMap(Project $project, string $methodology): array
    {
        ...
        $map = [];
        foreach ($generic as $k => $v) $map[$this->norm($k)] = $v;

        $methodMap = [];
        switch ($methodology) {
            case self::METH_SCRUM: $methodMap = $scrum; break;
            case self::METH_AGILE: $methodMap = $agile; break;
            case self::METH_WATERFALL: $methodMap = $waterfall; break;
            case self::METH_LEAN: $methodMap = $lean; break;
            default: $methodMap = [];
        }
        foreach ($methodMap as $k => $v) $map[$this->norm($k)] = $v;
        ...
        try {
            ...
            if (is_array($meta) && !empty($meta['status_aliases']) && is_array($meta['status_aliases'])) {
                ...
                foreach ($meta['status_aliases'] as $alias => $value) {
                    ...
                    if (in_array($valNorm, self::SERVER_STATUSES, true)) $server = $valNorm;
                    elseif (isset($phaseToServer[$valNorm])) $server = $phaseToServer[$valNorm];
                    elseif (isset($map[$valNorm])) $server = $map[$valNorm];

                    if ($server) $map[$aliasNorm] = $server;
                }
            }
        } catch (\Throwable $e) {}
    ...
    {
        $type = $this->normalizeType($plan['type'] ?? null);
        if (!$type) return ['_ok' => false, '_why' => 'Invalid command.'];

        $onlyAllowed = ['create_task','task_update','task_delete','bulk_update','bulk_assign','bulk_delete_overdue','bulk_delete_all','bulk_delete'];
        if (!in_array($type, $onlyAllowed, true)) return ['_ok' => false, '_why' => 'Unsupported command type.'];

        if ($type === 'task_update') {
            $id = (int)($plan['selector']['id'] ?? 0);
            if ($id <= 0) return ['_ok' => false, '_why' => 'A specific task id is required.'];
            $exists = Task::where('project_id',$project->id)->where('id',$id)->exists();
            if (!$exists) return ['_ok' => false, '_why' => "Task #{$id} not found in this project."];
            $chg = $plan['changes'] ?? [];
            if (empty($chg)) return ['_ok' => false, '_why' => 'No changes specified.'];
        }
        ...
        if ($type === 'bulk_update') {
            ...
            if (empty($updates)) return ['_ok' => false, '_why' => 'No updates specified.'];
            $count = $this->countAffected($project, $filters);
            if ($count <= 0) return ['_ok' => false, '_why' => 'No tasks match these filters.'];
        }

        if ($type === 'bulk_assign') {
            $filters = $plan['filters'] ?? [];
            $assignee = (string)($plan['assignee'] ?? '');
            if ($assignee === '') return ['_ok' => false, '_why' => 'Assignee is required.'];
            $count = $this->countAffected($project, $filters);
            if ($count <= 0) {
                if (!empty($filters['unassigned'])) return ['_ok'=>false,'_why'=>'No unassigned tasks found.'];
        ...
        if ($type === 'bulk_delete_overdue') {
            $count = $this->countOverdue($project);
            if ($count <= 0) return ['_ok' => false, '_why' => 'No overdue tasks found.'];
        }

        if ($type === 'bulk_delete_all') {
            $count = Task::where('project_id',$project->id)->count();
            if ($count <= 0) return ['_ok' => false, '_why' => 'No tasks to delete.'];
        }

        if ($type === 'create_task') {
            $title = trim((string)($plan['payload']['title'] ?? ''));
            if ($title === '') return ['_ok' => false, '_why' => 'Task title is required.'];
        }
    ...
    {
        $pieces = [];
        if (isset($updates['status'])) $pieces[] = "set status to \"".$this->prettyPhase($methodology, $updates['status'])."\"";
        if (isset($updates['priority'])) $pieces[] = "set priority to ".$updates['priority'];
        if (isset($updates['assignee_hint'])) $pieces[] = "assign to \"".$updates['assignee_hint']."\"";
        if (isset($updates['end_date'])) $pieces[] = "set due date to ".$updates['end_date'];
        if (isset($updates['start_date'])) $pieces[] = "set start date to ".$updates['start_date'];
        if (isset($updates['description'])) $pieces[] = (($updates['_mode'] ?? 'replace_desc') === 'append_desc') ? "append to description" : "replace description";
        if (isset($updates['title'])) $pieces[] = "rename title";
    ...
    {
        $parts = [];
        if (!empty($filters['ids'])) $parts[] = "with ids ".implode(', ', array_map('intval', $filters['ids']));
        if (!empty($filters['title_hints'])) $parts[] = "titled like \"".implode('", "', $filters['title_hints'])."\"";
        if (!empty($filters['title_contains'])) $parts[] = "title contains \"".implode('", "', $filters['title_contains'])."\"";
        if (!empty($filters['description_contains'])) $parts[] = "description contains \"".implode('", "', $filters['description_contains'])."\"";
        if (!empty($filters['status'])) $parts[] = "in \"".$this->prettyPhase($methodology, $filters['status'])."\"";
        if (!empty($filters['priority'])) $parts[] = "with priority ".$filters['priority'];
        if (!empty($filters['overdue'])) $parts[] = "that are overdue";
        if (!empty($filters['unassigned'])) $parts[] = "that are unassigned";
        if (!empty($filters['assigned_to_hint'])) $parts[] = "assigned to \"".$filters['assigned_to_hint']."\"";
        if (!empty($filters['due_before'])) $parts[] = "due before ".$filters['due_before'];
        if (!empty($filters['due_after'])) $parts[] = "due after ".$filters['due_after'];
        if (!empty($filters['due_on'])) $parts[] = "due on ".$filters['due_on'];
        if (!empty($filters['created_before'])) $parts[] = "created before ".$filters['created_before'];
        if (!empty($filters['created_after'])) $parts[] = "created after ".$filters['created_after'];
        if (!empty($filters['limit'])) {
            $dir = (!empty($filters['order']) && strtolower($filters['order']) === 'desc') ? 'last' : 'first';
            $parts[] = "{$dir} ".(int)$filters['limit']." task(s)";
        }
        if (empty($parts)) return "in this project";
    ...
    private function parseDate(string $text): ?Carbon
    {
        $text = trim($text);
        if ($text === '') return null;
        try { return Carbon::parse($text); } catch (\Throwable $e) { return null; }
    }
    ...
    private function normalizeLLMPlan(Project $project, array $plan): array
    {
        ...
        if (isset($plan['changes']) && is_array($plan['changes'])) {
            if (isset($plan['changes']['status'])) {
                ...
                if ($st) $plan['changes']['status'] = $st; else unset($plan['changes']['status']);
            }
            if (isset($plan['changes']['priority'])) {
                $pr = $normPrio($plan['changes']['priority']);
                if ($pr) $plan['changes']['priority'] = $pr; else unset($plan['changes']['priority']);
            }
        }
        ...
        if (isset($plan['filters']) && is_array($plan['filters'])) {
            if (isset($plan['filters']['status'])) {
                ...
                if ($st) $plan['filters']['status'] = $st; else unset($plan['filters']['status']);
            }
            if (isset($plan['filters']['priority'])) {
                $pr = $normPrio($plan['filters']['priority']);
                if ($pr) $plan['filters']['priority'] = $pr; else unset($plan['filters']['priority']);
            }
    ...
    private function handleDueDateQuery(Project $project, string $message): array
    {
        ...
        if (strpos($m, 'today') !== false) $window = 'today';
        elseif (strpos($m, 'tomorrow') !== false) $window = 'tomorrow';
        elseif (strpos($m, 'this week') !== false) $window = 'this week';
        elseif (strpos($m, 'next week') !== false) $window = 'next week';
        elseif (strpos($m, 'overdue') !== false) $window = 'overdue';
