...
class SimulationEngine
{
    ...
    public function advanceDay(VirtualProjectSimulation $simulation): void
    {
        ...
        // 3.5. Generate interactive tasks requiring user input (15% chance daily)
        if (rand(1,100) <= 15) {
            $this->generateInteractiveTasks($simulation);
        }
    ...
    /**
     * Generate interactive tasks requiring user input and evaluation
     */
    protected function generateInteractiveTasks(VirtualProjectSimulation $simulation): void
    {
        $domain = strtolower($simulation->meta['domain'] ?? 'general');
        $teamMembers = $simulation->teamMembers;
        ...
        $interactiveTasks = [
            // Email composition tasks
            'email_stakeholder_update' => [
                'title' => 'Draft stakeholder progress update email',
                'type' => 'email_composition',
                'context' => [
                    'recipient' => 'Project Stakeholders',
                    'purpose' => 'Weekly progress update',
                    'key_points' => ['Current progress', 'Recent milestones', 'Upcoming deliverables', 'Any blockers']
                ]
            ],
            'email_client_concern' => [
                'title' => 'Respond to client concern about timeline',
                'type' => 'email_composition',
                'context' => [
                    'recipient' => 'Primary Client Contact',
                    'purpose' => 'Address timeline concerns',
                    'key_points' => ['Acknowledge concern', 'Explain current status', 'Propose solutions', 'Next steps']
                ]
            ],
            'email_team_motivation' => [
                'title' => 'Send motivational message to team after setback',
                'type' => 'email_composition',
                'context' => [
                    'recipient' => 'Project Team',
                    'purpose' => 'Boost morale after challenges',
                    'key_points' => ['Acknowledge difficulties', 'Highlight progress', 'Reinforce vision', 'Support available']
                ]
            ],
            
            // Workflow automation tasks
            'workflow_notification_setup' => [
                'title' => 'Design automated notification workflow for task delays',
                'type' => 'workflow_design',
                'context' => [
                    'trigger' => 'Task overdue by 24+ hours',
                    'actions_needed' => ['Notify assignee', 'Alert team lead', 'Update dashboard', 'Log for review'],
                    'conditions' => ['Only during business hours', 'Skip weekends', 'Escalation rules']
                ]
            ],
            'workflow_approval_process' => [
                'title' => 'Create approval workflow for budget requests over $1000',
                'type' => 'workflow_design',
                'context' => [
                    'trigger' => 'Budget request submitted',
                    'actions_needed' => ['Route to appropriate approver', 'Send notifications', 'Track approval time', 'Log decisions'],
                    'conditions' => ['Different rules by amount', 'Backup approvers', 'Auto-rejection rules']
                ]
            ],
            
            // Task comment responses
            'respond_technical_question' => [
                'title' => 'Respond to technical question on high-priority task',
                'type' => 'task_comment',
                'context' => [
                    'task_title' => $teamMembers->random()->name . ' asked: "Should we use approach A or B for the database optimization?"',
                    'urgency' => 'High',
                    'technical_context' => 'Performance vs maintainability tradeoff decision needed'
                ]
            ],
            'address_scope_question' => [
                'title' => 'Clarify scope question from team member',
                'type' => 'task_comment',
                'context' => [
                    'task_title' => $teamMembers->random()->name . ' commented: "The requirements seem unclear on user permissions. Should we include admin roles?"',
                    'urgency' => 'Medium',
                    'decision_needed' => 'Scope clarification for user management features'
                ]
            ]
        ];

        // Domain-specific interactive tasks
        $domainTasks = [];

        $allTasks = array_merge($interactiveTasks, $domainTasks);
        
        if (empty($allTasks)) {
            return;
        }
        
        // Select 1-2 random interactive tasks
        $selectedKeys = collect(array_keys($allTasks))->random(rand(1, min(2, count($allTasks))));
        
        foreach ($selectedKeys as $key) {
            $taskDef = $allTasks[$key];
            
            SimulationTask::create([
                'simulation_id' => $simulation->id,
                'title' => $taskDef['title'],
                'estimated_hours' => rand(1, 4), // Interactive tasks are shorter
                'remaining_hours' => rand(1, 4),
                'priority' => collect(['medium', 'high'])->random(),
                'skill_tags' => ['communication', 'decision_making', 'leadership'],
                ...
                'meta' => json_encode([
                    ...
                    'evaluation_criteria' => $this->getEvaluationCriteria($taskDef['type'])
                ])
            ]);
        }

        if (!empty($selectedKeys)) {
            $this->createEvent($simulation, 'interactive_tasks_generated', [
                'tasks_count' => count($selectedKeys),
                'task_types' => collect($selectedKeys)->map(fn($key) => $allTasks[$key]['type'])->toArray(),
                'domain_focus' => $domain
            ]);
        }
    ...
    /**
     * Get evaluation criteria for different interactive task types
     */
    protected function getEvaluationCriteria(string $taskType): array
    {
        return match($taskType) {
            'email_composition' => [
                'clarity' => 'Is the message clear and easy to understand?',
                'tone' => 'Is the tone appropriate for the recipient and situation?',
                'completeness' => 'Are all key points addressed?',
                'professionalism' => 'Is the email professionally formatted and written?',
                'action_orientation' => 'Does it clearly communicate next steps or expectations?'
            ],
            'workflow_design' => [
                'completeness' => 'Are all necessary steps included?',
                'efficiency' => 'Is the workflow streamlined and efficient?',
                'error_handling' => 'Are edge cases and errors handled?',
                'scalability' => 'Will this work as the project/team grows?',
                'user_experience' => 'Is it easy for users to follow?'
            ],
            'task_comment' => [
                'helpfulness' => 'Does the response help move the task forward?',
                'technical_accuracy' => 'Is the technical guidance sound?',
                'clarity' => 'Is the response clear and actionable?',
                'timeliness' => 'Is the response appropriate for the urgency level?',
                'collaboration' => 'Does it foster good team collaboration?'
            ],
            default => [
                'quality' => 'Overall quality of the response',
                'relevance' => 'Relevance to the task and context',
    ...
    {
        $eventTypes = [
            'requirement_change' => ['note' => 'Stakeholder adds export reports feature', 'added_hours' => rand(6,14)],
            'scope_creep' => ['note' => 'Minor UX polish requests accumulate', 'percent_increase' => 10],
            'dependency_block' => ['blocked_area' => collect(['Payments API','Auth service','3rd-party analytics'])->random(), 'days' => rand(1,3)],
            'budget_cut' => ['cut' => rand(1000,4000)],
            'vendor_delay' => ['vendor' => collect(['Email provider','Cloud hosting','Payment gateway'])->random(), 'days' => rand(2,5)],
            'performance_issue' => ['area' => collect(['Search queries','Checkout flow','Dashboard load'])->random(), 'investigation_hours' => rand(4,10)],
            'security_alert' => ['severity' => collect(['low','medium','high'])->random()],
            'team_conflict' => ['note' => 'Misalignment between frontend & backend expectations','morale_drop' => rand(5,15)],
            'client_feedback' => ['concern' => collect(['UI not intuitive','Performance concerns','Missing features','Integration issues'])->random(), 'urgency' => collect(['low','medium','high'])->random()],
        ];
        
        $chosenKey = collect(array_keys($eventTypes))->random();
        $payload = $eventTypes[$chosenKey];
        $this->createEvent($simulation, $chosenKey, $payload);
        
        // Apply effects based on event type
        switch ($chosenKey) {
            case 'requirement_change':
                $this->inflateRandomTasks($simulation, 2, rand(2,6));
                break;
            case 'scope_creep':
                $this->percentAdjustAllPending($simulation, 0.10);
                break;
            case 'budget_cut':
                $simulation->budget_total = max(0, $simulation->budget_total - $payload['cut']);
                $simulation->save();
                break;
            case 'performance_issue':
                $this->injectNewTask($simulation, 'Investigate performance regression');
                break;
            case 'security_alert':
                $this->injectNewTask($simulation, 'Apply security patch & review dependencies', 'high');
                break;
        }
    }
