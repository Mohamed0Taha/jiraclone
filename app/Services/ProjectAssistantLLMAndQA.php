...
trait ProjectAssistantLLMAndQA
{
    private function buildEnhancedSnapshot(Project $project): array
    {
        ...
        foreach ($project->tasks as $task) {
            ...
            $byStatus[$status]++; $byPriority[$priority]++;

            if (!empty($task->end_date) && $status !== 'done') {
                try { if (Carbon::parse($task->end_date)->isPast()) $overdue++; } catch (Throwable $e) {}
            }
            ...
            $taskDetails[] = [
                ...
                'is_overdue'=> !empty($task->end_date) && $status!=='done' && (function($d){ try {return Carbon::parse($d)->isPast();} catch(Throwable $e){return false;} })($task->end_date),
            ];
        }
...
SYS;
    ...
    private function getTaskAssignmentInfo(Project $project, int $taskId): array
    {
        ...
        if (!$task) return $this->createResponse('information', "Task #{$taskId} not found in this project.", false);
        if (!$task->assignee_id) return $this->createResponse('information', "Task #{$taskId} \"{$task->title}\" is currently unassigned.", false);
    ...
    private function getTaskCreatorInfo(Project $project, int $taskId): array
    {
        $task = Task::where('project_id', $project->id)->where('id', $taskId)->first();
        if (!$task) return $this->createResponse('information', "Task #{$taskId} not found in this project.", false);
        if (!$task->creator_id) return $this->createResponse('information', "Task #{$taskId} \"{$task->title}\" has no recorded creator.", false);
    ...
    private function getTasksByDuePeriod(Project $project, string $period): array
    {
        ...
        if (!$dateRange) return $this->createResponse('information', 'Invalid time period specified.', false);
        ...
        if ($count === 0) return $this->createResponse('information', "No tasks due {$periodLabel}.", false);

        $message = "{$count} task".($count === 1 ? '' : 's')." due {$periodLabel}:\n\n";
        foreach ($tasks->take(10) as $task) {
            $assignee = 'Unassigned';
            if ($task->assignee_id) { $user = User::find($task->assignee_id); $assignee = $user ? $user->name : "User #{$task->assignee_id}"; }
        ...
        if ($count > 10) $message .= "... and " . ($count - 10) . " more tasks";
    ...
    private function getTasksByCreatedPeriod(Project $project, string $period): array
    {
        ...
        if ($count === 0) return $this->createResponse('information', "No tasks created {$periodLabel}.", false);

        $message = "{$count} task".($count === 1 ? '' : 's')." created {$periodLabel}:\n\n";
        foreach ($tasks->take(10) as $task) {
            $creator = 'Unknown';
            if ($task->creator_id) { $user = User::find($task->creator_id); $creator = $user ? $user->name : "User #{$task->creator_id}"; }
    ...
    private function getOverdueTasks(Project $project): array
    {
        ...
        $count = $tasks->count();
        if ($count === 0) return $this->createResponse('information', "No overdue tasks found.", false);
    ...
    private function handleMilestoneQuery(Project $project, string $message): array
    {
        try {
            if (method_exists($project, 'milestones')) {
                ...
                if ($milestones->isEmpty()) return $this->createResponse('information', 'No milestones found in this project.', false);
    ...
    {
        $members = $this->projectMembers($project);
        if ($members->isEmpty()) return $this->createResponse('information', 'This project has no listed members.', false);
    ...
    private function countOverdue(Project $project): int
    {
        ...
        foreach ($q->get() as $t) { try { if (Carbon::parse($t->end_date)->isPast()) $count++; } catch (\Throwable $e) {} }
        return $count;
    }
}
