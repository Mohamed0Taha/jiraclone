...
class CommandProcessingService
{
    ...
    private function parseTaskUpdates(Project $project, string $message, int $taskId): array
    {
        ...
        if (preg_match('/\bpriority\b/i', $m) && preg_match('/\b(low|medium|high|urgent|critical|blocker|p[0-3])\b/i', $m, $matches)) {
            if ($priority = $this->resolvePriorityToken($matches[1])) $updates['priority'] = $priority;
        }

        if (preg_match('/\b(?:move|set|mark|status)\b.*?\b(?:to|in|as)\s+([a-z\- ]+)\b/i', $m, $matches)) {
            if ($status = $this->resolveStatusToken($project, $matches[1])) $updates['status'] = $status;
        }
    ...
    private function parseBulkUpdates(Project $project, string $message): array
    {
        ...
        if (preg_match('/\bmove\s+.*?\bto\s+([a-z\- ]+)/i', $m, $matches)) {
            if ($toStatus = $this->resolveStatusToken($project, end($matches))) $updates['status'] = $toStatus;
        }
    ...
    private function parseFiltersEnhanced(Project $project, string $message): array
    {
        ...
        if (preg_match_all('/#(\d+)/', $message, $matches)) $filters['ids'] = array_map('intval', $matches[1]);
        if (preg_match('/\b(low|medium|high|urgent)\s+priority\b/i', $m, $matches)) $filters['priority'] = $this->resolvePriorityToken($matches[1]);
        if (preg_match('/\bstatus\s+(?:is\s+)?([a-z\- ]+)/i', $m, $matches)) $filters['status'] = $this->resolveStatusToken($project, $matches[1]);
        if (strpos($m, 'overdue') !== false) $filters['overdue'] = true;
        if (strpos($m, 'unassigned') !== false) $filters['unassigned'] = true;

        if (preg_match('/\b(my|me)\b/i', $m)) $filters['assigned_to_hint'] = '__ME__';
        if (preg_match("/\bowner'?s\b/i", $m)) $filters['assigned_to_hint'] = '__OWNER__';
        if (preg_match("/\b([A-Za-z]+)'s\s+tasks\b/u", $message, $mm)) $filters['assigned_to_hint'] = trim($mm[1]);
        if (preg_match('/\bfor\s+(@?[a-z0-9._\- ]+)/i', $m, $mm)) $filters['assigned_to_hint'] = ltrim(trim($mm[1]), '@');
    ...
    private function validatePlan(Project $project, array $plan): array
    {
        ...
        if (!$type) return ['_ok' => false, '_why' => 'I couldn\'t understand that command. Please be more specific.'];
        ...
        if ($type === 'task_update' || $type === 'task_delete') {
            ...
            if ($id <= 0) return ['_ok' => false, '_why' => 'A specific task ID (e.g., #123) is required for this action.'];
            ...
            if (!$taskExists) return ['_ok' => false, '_why' => "Task #{$id} was not found in this project."];
            if ($type === 'task_update' && empty($plan['changes'])) return ['_ok' => false, '_why' => 'Please specify what to change (e.g., "set priority to high").'];
        }
        
        if (in_array($type, ['bulk_update', 'bulk_assign', 'bulk_delete'])) {
            $filters = $plan['filters'] ?? [];
            if (empty($filters)) return ['_ok' => false, '_why' => 'Please specify which tasks to affect (e.g., "all overdue tasks").'];
            if ($this->contextService->countAffected($project, $filters) <= 0) return ['_ok' => false, '_why' => 'No tasks match the specified filters.'];
            if ($type === 'bulk_update' && empty($plan['updates'])) return ['_ok' => false, '_why' => 'Please specify what to update (e.g., "move to done").'];
            if ($type === 'bulk_assign' && empty($plan['assignee'])) return ['_ok' => false, '_why' => 'Please specify who to assign the tasks to.'];
        }
    ...
    {
        $parts = [];
        if (!empty($filters['all'])) return "on ALL tasks";
        if (!empty($filters['status'])) $parts[] = "in \"".$this->prettyPhase($methodology, $filters['status'])."\"";
        if (!empty($filters['priority'])) $parts[] = "with ".$filters['priority']." priority";
    ...
    private function normalizeLLMPlan(Project $project, array $plan): array
    {
        ...
        foreach (['changes', 'payload', 'updates', 'filters'] as $key) {
            if (isset($plan[$key]) && is_array($plan[$key])) {
                if (isset($plan[$key]['status'])) $plan[$key]['status'] = $normStatus($plan[$key]['status']);
                if (isset($plan[$key]['priority'])) $plan[$key]['priority'] = $normPrio($plan[$key]['priority']);
            }
        }
        if (!empty($plan['filters']['assigned_to_hint'])) unset($plan['filters']['all']);
    ...
    private function resolveStatusToken(Project $project, string $token): ?string
    {
        ...
        if (in_array($t, self::SERVER_STATUSES)) return $t;
    ...
    {
        $t = $this->norm($token);
        if (in_array($t, self::PRIORITIES)) return $t;
    ...
    {
        return match ($ordinal) { 'first' => 'todo', 'second' => 'inprogress', 'third' => 'review', 'fourth' => 'done', default => null };
    }
    ...
    private function parseOrdinalWindow(string $mLow): ?array
    {
        if (preg_match('/\b(first|last|top)\s+(one|two|three|four|five|\d+)\b/i', $mLow, $mm)) {
            ...
            if ($n > 0) return ['limit' => $n, 'order' => ($mm[1] === 'last') ? 'desc' : 'asc'];
        }
        return null;
    }
    ...
    private function countOverdue(Project $project): int
    {
        ...
        foreach ($q->get() as $task) {
            try {
                if (Carbon::parse($task->end_date)->isPast()) $count++;
            } catch (\Throwable $e) {
    ...
    private function createTaskPlan(string $title): array { return ['type' => 'create_task', 'payload' => ['title' => $title, 'status' => 'todo', 'priority' => 'medium']]; }
    private function createTaskUpdatePlan(int $id, array $ch): array { return ['type' => 'task_update', 'selector' => ['id' => $id], 'changes' => $ch]; }
    private function createBulkUpdatePlan(array $f, array $u): array { return ['type' => 'bulk_update', 'filters' => $f, 'updates' => $u]; }
}