<?php

namespace App\Services;

use App\Models\SmsMessage;
use Illuminate\Support\Facades\Log;
use Twilio\Exceptions\TwilioException;
use Twilio\Rest\Client;

class TwilioService
{
    protected Client $twilio;

    protected string $phoneNumber;

    public function __construct()
    {
        $accountSid = config('twilio.account_sid');
        $authToken = config('twilio.auth_token');

        if (! $accountSid || ! $authToken) {
            throw new \Exception('Twilio credentials not configured. Please set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN');
        }

        $this->twilio = new Client($accountSid, $authToken);
        Log::info('Twilio initialized with Auth Token authentication', [
            'account_sid' => substr($accountSid, 0, 8).'...',
        ]);

        $this->phoneNumber = config('twilio.phone_number');

        Log::info('TwilioService initialized', [
            'phone_number' => $this->phoneNumber,
        ]);
    }

    /**
     * Send SMS message with database logging
     */
    public function sendSMS(string $to, string $message, array $options = []): array
    {
        // Create SMS record first
        $smsMessage = SmsMessage::create([
            'user_id' => $options['user_id'] ?? null,
            'automation_id' => $options['automation_id'] ?? null,
            'from_number' => $this->phoneNumber,
            'to_number' => $to,
            'body' => $message,
            'status' => 'queued',
        ]);

        if (! config('twilio.sms_enabled')) {
            Log::info('SMS messaging disabled in configuration');
            $smsMessage->update([
                'status' => 'failed',
                'error_message' => 'SMS messaging is disabled',
            ]);

            return ['success' => false, 'error' => 'SMS messaging is disabled'];
        }

        if (! $this->phoneNumber) {
            Log::error('Twilio phone number not configured');
            $smsMessage->update([
                'status' => 'failed',
                'error_message' => 'Twilio phone number not configured',
            ]);

            return ['success' => false, 'error' => 'Twilio phone number not configured'];
        }

        try {
            // Format phone number
            $to = $this->formatPhoneNumber($to);

            // Truncate message if too long
            $maxLength = config('twilio.max_message_length', 160);
            if (strlen($message) > $maxLength) {
                $message = substr($message, 0, $maxLength - 3).'...';
                Log::warning('SMS message truncated to fit limit', ['max_length' => $maxLength]);
            }

            $messageData = [
                'from' => $this->phoneNumber,
                'body' => $message,
            ];

            // Add optional parameters
            if (isset($options['statusCallback'])) {
                $messageData['statusCallback'] = $options['statusCallback'];
            }

            if (isset($options['mediaUrl'])) {
                $messageData['mediaUrl'] = $options['mediaUrl'];
            }

            $twilioMessage = $this->twilio->messages->create($to, $messageData);

            // Update SMS record with success data
            $smsMessage->update([
                'twilio_sid' => $twilioMessage->sid,
                'status' => $twilioMessage->status,
                'price' => $twilioMessage->price ? (float) $twilioMessage->price : null,
                'price_unit' => $twilioMessage->priceUnit,
                'direction' => $twilioMessage->direction ?? 'outbound',
            ]);

            Log::info('SMS message sent successfully', [
                'to' => $to,
                'from' => $this->phoneNumber,
                'message_sid' => $twilioMessage->sid,
                'status' => $twilioMessage->status,
                'sms_record_id' => $smsMessage->id,
            ]);

            return [
                'success' => true,
                'message_sid' => $twilioMessage->sid,
                'status' => $twilioMessage->status,
                'to' => $to,
                'from' => $this->phoneNumber,
                'body' => $message,
                'sms_record_id' => $smsMessage->id,
            ];

        } catch (TwilioException $e) {
            // Update SMS record with error
            $smsMessage->update([
                'status' => 'failed',
                'error_message' => $e->getMessage(),
                'error_code' => $e->getCode(),
            ]);

            Log::error('Twilio SMS error', [
                'error_code' => $e->getCode(),
                'error_message' => $e->getMessage(),
                'to' => $to,
                'from' => $this->phoneNumber,
                'sms_record_id' => $smsMessage->id,
            ]);

            return [
                'success' => false,
                'error' => $e->getMessage(),
                'error_code' => $e->getCode(),
                'sms_record_id' => $smsMessage->id,
            ];
        }
    }

    /**
     * Send message via SMS
     */
    public function sendMessage(string $to, string $message, array $options = []): array
    {
        return $this->sendSMS($to, $message, $options);
    }

    /**
     * Update SMS message status (called by webhooks)
     */
    public function updateMessageStatus(string $messageSid, string $status, array $data = []): bool
    {
        $smsMessage = SmsMessage::where('twilio_sid', $messageSid)->first();

        if (! $smsMessage) {
            Log::warning('SMS message not found for status update', ['message_sid' => $messageSid]);

            return false;
        }

        $updateData = ['status' => $status];

        if (isset($data['price'])) {
            $updateData['price'] = (float) $data['price'];
        }

        if (isset($data['price_unit'])) {
            $updateData['price_unit'] = $data['price_unit'];
        }

        if (isset($data['error_message'])) {
            $updateData['error_message'] = $data['error_message'];
        }

        if (isset($data['error_code'])) {
            $updateData['error_code'] = (int) $data['error_code'];
        }

        $updateData['webhook_data'] = $data;

        $smsMessage->update($updateData);

        Log::info('SMS message status updated', [
            'message_sid' => $messageSid,
            'status' => $status,
            'sms_record_id' => $smsMessage->id,
        ]);

        return true;
    }

    /**
     * Format phone number (ensure it has country code)
     */
    protected function formatPhoneNumber(string $phoneNumber): string
    {
        // Remove any whitespace and special characters except +
        $phoneNumber = preg_replace('/[^\d+]/', '', $phoneNumber);

        // If it doesn't start with +, add default country code
        if (! str_starts_with($phoneNumber, '+')) {
            $defaultCountryCode = config('twilio.default_country_code', '+1');
            $phoneNumber = $defaultCountryCode.$phoneNumber;
        }

        return $phoneNumber;
    }

    /**
     * Get account information
     */
    public function getAccountInfo(): array
    {
        try {
            $account = $this->twilio->api->v2010->accounts(config('twilio.account_sid'))->fetch();

            return [
                'success' => true,
                'account_sid' => $account->sid,
                'friendly_name' => $account->friendlyName,
                'status' => $account->status,
                'type' => $account->type,
            ];
        } catch (TwilioException $e) {
            return [
                'success' => false,
                'error' => $e->getMessage(),
            ];
        }
    }

    /**
     * Get phone numbers associated with account
     */
    public function getPhoneNumbers(): array
    {
        try {
            $phoneNumbers = $this->twilio->incomingPhoneNumbers->read();

            $numbers = [];
            foreach ($phoneNumbers as $number) {
                $numbers[] = [
                    'phone_number' => $number->phoneNumber,
                    'friendly_name' => $number->friendlyName,
                    'capabilities' => [
                        'sms' => $number->capabilities['sms'] ?? false,
                        'voice' => $number->capabilities['voice'] ?? false,
                    ],
                ];
            }

            return [
                'success' => true,
                'phone_numbers' => $numbers,
            ];
        } catch (TwilioException $e) {
            return [
                'success' => false,
                'error' => $e->getMessage(),
            ];
        }
    }

    /**
     * Validate phone number format
     */
    public function validatePhoneNumber(string $phoneNumber): bool
    {
        $formatted = $this->formatPhoneNumber($phoneNumber);

        return preg_match('/^\+\d{10,15}$/', $formatted) === 1;
    }

    /**
     * Sync SMS message status from Twilio
     */
    public function syncMessageStatus(SmsMessage $smsMessage): bool
    {
        if (! $smsMessage->twilio_sid) {
            Log::warning('Cannot sync status: SMS message has no Twilio SID', ['sms_id' => $smsMessage->id]);

            return false;
        }

        try {
            $twilioMessage = $this->twilio->messages($smsMessage->twilio_sid)->fetch();

            $smsMessage->update([
                'status' => $twilioMessage->status,
                'price' => $twilioMessage->price ? (float) $twilioMessage->price : $smsMessage->price,
                'price_unit' => $twilioMessage->priceUnit ?: $smsMessage->price_unit,
                'error_code' => $twilioMessage->errorCode,
                'error_message' => $twilioMessage->errorMessage,
                'date_sent' => $twilioMessage->dateSent ? $twilioMessage->dateSent->format('Y-m-d H:i:s') : null,
                'date_updated' => $twilioMessage->dateUpdated ? $twilioMessage->dateUpdated->format('Y-m-d H:i:s') : null,
            ]);

            Log::info('SMS message status synced from Twilio', [
                'sms_id' => $smsMessage->id,
                'twilio_sid' => $smsMessage->twilio_sid,
                'old_status' => $smsMessage->getOriginal('status'),
                'new_status' => $twilioMessage->status,
            ]);

            return true;
        } catch (TwilioException $e) {
            Log::error('Failed to sync SMS status from Twilio', [
                'sms_id' => $smsMessage->id,
                'twilio_sid' => $smsMessage->twilio_sid,
                'error' => $e->getMessage(),
            ]);

            return false;
        }
    }

    /**
     * Sync all pending SMS messages status from Twilio
     */
    public function syncAllPendingMessages(): array
    {
        $pendingMessages = SmsMessage::whereIn('status', ['queued', 'sent', 'accepted'])
            ->whereNotNull('twilio_sid')
            ->get();

        $results = [
            'total' => $pendingMessages->count(),
            'synced' => 0,
            'failed' => 0,
        ];

        foreach ($pendingMessages as $message) {
            if ($this->syncMessageStatus($message)) {
                $results['synced']++;
            } else {
                $results['failed']++;
            }
        }

        Log::info('Bulk SMS status sync completed', $results);

        return $results;
    }

    /**
     * Get service configuration
     */
    public function getConfig(): array
    {
        return [
            'phone_number' => $this->phoneNumber,
            'sms_enabled' => config('twilio.sms_enabled'),
            'max_message_length' => config('twilio.max_message_length'),
            'default_country_code' => config('twilio.default_country_code'),
        ];
    }
}
