...
class ProjectController extends Controller
{
    ...
    {
        $lines = [];
        if (! empty($meta['project_type'])) {
            $lines[] = 'Type: '.$meta['project_type'];
        }
        if (! empty($meta['domain'])) {
            $lines[] = 'Domain: '.$meta['domain'];
        }
        if (! empty($meta['area'])) {
            $lines[] = 'Area: '.$meta['area'];
        }
        if (! empty($meta['location'])) {
            $lines[] = 'Location: '.$meta['location'];
        }
        if (! empty($meta['team_size'])) {
            $lines[] = 'Team size: '.$meta['team_size'];
        }
        if (! empty($validated['start_date'])) {
            $lines[] = 'Start: '.$validated['start_date'];
        }
        if (! empty($validated['end_date'])) {
            $lines[] = 'End: '.$validated['end_date'];
        }
        if (! empty($meta['budget'])) {
            $lines[] = 'Budget: '.$meta['budget'];
        }
        if (! empty($meta['primary_stakeholder'])) {
            $lines[] = 'Primary stakeholder: '.$meta['primary_stakeholder'];
        }
        if (! empty($meta['objectives'])) {
            $lines[] = 'Objectives: '.$meta['objectives'];
        }
    ...
    // Export tasks in Jira CSV format
    public function exportJira(Project $project)
    {
        $this->authorize('view', $project);
